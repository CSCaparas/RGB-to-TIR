...
def on_frame(camera, camera_frame, stuff):
    ...
    # any cropping of thermal image here

    # Append the frame to the CSV file.
    # np.savetxt(stuff[0], frame.data, fmt="%.1f")
    # change to making a new csv

    # Save the RGB image
    ret,ogrgb = stuff[1].read()
    rgbimg = ogrgb#cv2.flip(ogrgb, 1)#flip horizontally
    rgbimg = cv2.rotate(rgbimg, cv2.ROTATE_90_CLOCKWISE) # any adjustments here
    rgbimg = rgbimg[64:576, 0:512] # any cropping changes here
    rgbimg = cv2.resize(rgbimg, (240,240), interpolation = cv2.INTER_AREA)

    max_number = 0 # change this higher to start count above previous
    ...
    therm_filename = str(max_number + 1) + ".csv"
    np.savetxt(therm_filename, frame.data, fmt="%.1f")
...
def main():
    ...
    rgb = cv2.VideoCapture(0) # camera choice
    sleep(0.25)
